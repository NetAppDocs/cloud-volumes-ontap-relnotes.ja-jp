...
. ^5^ 次世代C3ファミリーVMをCloud Volumes ONTAP 9.18.1以降に導入できます。c3-standard-44およびc3-standard-88マシンとそのLSSDバリアントは、デフォルトでGoogle Cloud ConsoleのTier-1帯域幅をサポートします。C3 VMタイプでは、既存のアグリゲートにディスクを追加したり、C3ファミリ内の別のバリアントに切り替えたりすることはできません。LSSDバリアントの詳細については、link:reference-configs-gcp.html#flash-cache-support["Flash Cacheサポート"]を参照してください。
...
Cloud Volumes ONTAP 9.18.1以降、C3 VMのローカルSSD（LSSD）バリアントを選択すると、Flash Cacheがデフォルトで有効になります。VMバリアントとそれによってサポートされるLSSDの最大数を確認するには、次の表を確認してください：
