...
. ^5^ 次世代 C3 ファミリー VM を https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/whats-new.html#improved-cloud-volumes-ontap-support-with-google-cloud-c3-vms["Cloud Volumes ONTAP 9.18.1以降"^]にデプロイできます。c3-standard-44 および c3-standard-88 マシンとそのLSSDバリアントは、Google Cloud Console でデフォルトで Tier-1 帯域幅をサポートしています。LSSDバリアントの詳細については、link:reference-configs-gcp.html#flash-cache-support["Flash Cacheサポート"]を参照してください。
...
. ^3^Balanced Persistent DiskおよびPerformance（SSD）Persistent Diskを使用すると、書き込みパフォーマンスの向上が有効になります。
. ^4^ 2025 年 8 月 11 日以降、 Cloud Volumes ONTAP Optimized ライセンスは非推奨となり、従量課金制 (PAYGO) サブスクリプションの Google Cloud マーケットプレイスで購入または更新できなくなります。詳細については、 https://docs.netapp.com/us-en/bluexp-cloud-volumes-ontap/whats-new.html["Cloud Volumes ONTAP の新機能"^] 。
. ^5^ 次世代 C3 ファミリー VM を https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/whats-new.html#support-for-next-generation-google-cloud-vms-in-cloud-volumes-ontap-ha["Cloud Volumes ONTAP 9.19.1以降（シングルAZ HAデプロイメントの場合）"^]にデプロイできます。c3-standard-44 および c3-standard-88 マシンとその LSSD バリアントは、Google Cloud Console でデフォルトで Tier-1 帯域幅をサポートしています。LSSD バリアントの詳細については、link:reference-configs-gcp.html#flash-cache-support["Flash Cacheサポート"]を参照してください。
...
[cols="4*"]
|===
| VMタイプ | シングルノード展開でサポートされる最大データディスク数 | 単一AZ HA展開でサポートされる最大データディスク数 | 最大容量（TiB単位） 
