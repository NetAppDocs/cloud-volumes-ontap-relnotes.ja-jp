...
---
= AWSにおけるCloud Volumes ONTAPの既知の制限事項
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
AWS Outpost がある場合は、デプロイ中に Outpost VPC を選択することで、その Outpost にCloud Volumes ONTAP をデプロイできます。エクスペリエンスは、AWS にある他の VPC と同じです。最初に AWS Outpost にコンソールエージェントをデプロイする必要があることに注意してください。
...
+
NetAppコンソールからボリュームを作成するときにストレージ効率を選択しないことも、ボリュームを作成してから http://docs.netapp.com/ontap-9/topic/com.netapp.doc.dot-cm-vsmg/GUID-8508A4CB-DB43-4D0D-97EB-859F58B29054.html["CLI を使用してデータ圧縮を無効にします"^]。
