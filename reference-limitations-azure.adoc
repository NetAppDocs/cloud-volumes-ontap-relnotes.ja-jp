...
Cloud Volumes ONTAPは、 Azure仮想マシン（VM）拡張機能をサポートしていません。これは、操作を遅延させるためです。CloudCloud Volumes ONTAPの導入後、 BlueXPは定期的にVM拡張機能を無効化します。これらの拡張機能を作業環境にインストールしている場合は、Microsoft Azureサポートに連絡して無効化または削除してください。
